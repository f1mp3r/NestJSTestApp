...
### 7.1 Multi language module

Configuration is made via .env file with these 2 lines:
```bash
APP_LANG=en
ALL_LANGUAGES=en,bg
``` 
where we define all used languages and selected currently language.

In order to use multi language module add following row at the header to import the module
```sh
import * as i18n from 'i18n';
```
then into the controller or anywhere use as example:
```sh
i18n.__('Welcome to the site'); 
```

If the text is not into the translate files from /src/locales that record will be crated automatically from the module.
